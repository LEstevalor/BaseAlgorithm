...
public class MinTreeTest {
    public static void main(String[] args) {
        ...
        minTree.createGraph(graph, nodeCount, data, near);
...
class MinTree {
    //创建图的邻接矩阵
    public void createGraph(Graph graph, int nodeCount, char[] data, int[][] near) {
        for (int k = 0; k < nodeCount; k++) {
            graph.data[k] = data[k];
            for (int l = 0; l < nodeCount; l++) {
                graph.near[k][l] = near[k][l];
            }
        }
    }
...
class Graph {
    ...
    public Graph(int nodeCount) {
        ...
        data = new char[nodeCount];
        near = new int[nodeCount][nodeCount];
    }
}
